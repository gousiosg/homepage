...
in particular by @headinthebox. Reactive programming is a set of techniques to
...
The discussion on streams (or signals, as known in other contexts)
however blurs the practicality of reactive programming. The practice
boils down to creating `Observable` sequences of events (e.g. by poling
an API or waiting on a socket) and then assigning actions to incoming events.
The interesting situations start when we combine several streams, in a
time-dependent fashion: say that we want an action to occur as a result
of polling two APIs and combining the responses based on a common attribute.
Luckily, the complicated interactions are hidden quite well behind the
Rx API; powerful operators such as `Merge` that merges two streams
or `Buffer` that creates a time window of events before firing the event
...
public static async Task<IEnumerable<GithubInfo>> ReadGithubAsync(string user, string password) {
    ...
    var logins = (from e in JArray.Parse(json).Children()
            where (string)e["type"] == "PushEvent"
            let commits =  e["payload"]["commits"].Select (c => (string)c["sha"]).ToArray()
            select new GithubInfo{User = (string)e["actor"]["login"], Commits = commits});
    return logins.Where(l => l.Commits.Length != 0);
}
...
continuation passing style hidden by a very clever compiler trick. The
...
of modern web APIs. What's more is that the simplicity of this approach
makes event-based frameworks such as node.js, EventMachine and Gevent look
naive and even more complex.
...
public static async Task<IEnumerable<string>> ReadTwitterAsync(string user)
{
    ...
    var tweets = from e in JValue.Parse(json)["results"]
                 where (string)e["from_user"] == user
                  && ((string)e["text"]).IndexOf("github.com/.*/pulls/") > 0
...
requester, read his Tweeter stream for *some time* and return all tweets that
contain pull requests. There emphasized terms highlight the several time
...
The
big question now is how we go from one the first timeline to the second.  LINQ
to the resque!
...
public static IObservable<IEnumerable<GithubInfo>> pollGithub(TimeSpan interval)
{
    return Observable.Timer(TimeSpan.Zero, TimeSpan.FromSeconds(6)).SelectMany( _ =>
        ReadGithubAsync("foo", "bar")
    );
}

public static IObservable<string> pollTwitter(string user)
{
...
interested for a result every time we observe a new tweet; this is what
...
first call to `pollGithub`. What we need to do is to come up with an updatable
version of the static Tweeter marble diagram, where each new Github user
...
{% highlight csharp %}
public static
  IObservable<IGroupedObservable<String, GithubInfo>>
  GroupGithubUsers()
{
    var u = from users in pollGithub()
...
`GroupedObservable`s, where each group is identified by a user. This way we both
solve the problem of users issuing multiple user pull requests and create an
observable group (and therefore list, if we just get the group keys) of users to
read tweets from. Neat, eh?

Finally, combining the two streams (groups of users, and tweets for a user) is
again a job for LINQ:

{% highlight csharp%}
from ghinfo in GroupGithubUsers()
from u in ghinfo
from t in pollTwitter(u.User).TakeUntil(Observable.Timer(TimeSpan.FromMinutes(5)))
select new {User = u.User, Tweet = t};
{% endhighlight %}

The only difference to the static version is the fact that we stop
scanning for new user tweets after 5 minutes. To sum up, our example will poll
Github every 6 seconds for new pull requests asynchronously, create observable
sequences of pull requests grouped by username, create observable sequences of
tweets by polling Tweeter asynchronously every 10 seconds using the observable
user groups as source per user name and return a list of user names and tweets
that refer to pull requests. The code is fully asynchronous and in fact executed
using a thread pool for running the async tasks, so automatically parallelizable.

All this, in just 95 lines of code. Amazing, right?
