...
if ($ARGV[0] =~ m/\.bib$/) {
    ...
    if (! -e $bibfile) {
      copy ("$BIBDIR/$bibfile", "$bibfile");
      push(@tmpfiles, $bibfile);  
    }
...
<meta name="Generator" content="$Id: bib2xhtml 2.31 2010/02/04 14:31:08 dds Exp $" />
</head>
<body>
}
}
...
$currDir = `pwd`;
print STDERR "Current Dir:" . $currDir;
$auxfile = "bib$$";
...
print HTMLFILE <<EOF;
<!--Automatically maintained by bib2xhtml-->
EOF
